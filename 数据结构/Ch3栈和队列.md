...
| 类型                 | 示例            | 运算符位置 | 特点               |
| -------------------- | --------------- | ---------- | ------------------ |
| 中缀表达式           | `a+b*(c-d)-e/f` | 操作数中间 | 需考虑优先级和括号 |
| 前缀表达式(波兰式)   | `-+a*b-cd/ef`   | 操作数前面 | 从右向左计算       |
| 后缀表达式(逆波兰式) | `abcd-*+ef/-`   | 操作数后面 | 从左向右计算       |
...
string infixToPostfix(const string &infix) {
    ...
    for (char c : infix) {
        ...
        if (isalnum(c)) {                    // 支持多位数和变量
            result << c;
            while (isalnum(infix[infix.find(c) + 1])) {
                result << infix[infix.find(c) + 1];
            }
            result << ' ';
        }
        else if (c == '(') {
        ...
        else if (precedence.count(c)) {       // 是运算符
            while (!ops.empty() && ops.top() != '(' && 
...
double evaluateRPN(const string &postfix) {
    ...
    auto isNumber = [](const string& s) {
        return !s.empty() && (isdigit(s[0]) || 
               (s[0] == '-' && s.length() > 1 && isdigit(s[1])));
    };
    
    while (ss >> token) {
        if (isNumber(token)) {                // 处理负数
            stk.push(stod(token));
        } else if (token.length() == 1) {     // 单字符运算符
            if (stk.size() < 2) {
                cerr << "表达式错误：操作数不足" << endl;
                return 0;
            }
