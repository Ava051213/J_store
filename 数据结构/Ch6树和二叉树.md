...
class BinaryTree {
  ...
  private:
    ...
    void destory(TreeNode<T> *&node) {
        if (node) {
            destory(node->left), destory(node->right);
            delete node;
            node = nullptr;
        }
    }
...
struct ThreadNode {
    int data;
    ThreadNode *lchild; // 数据域
    ThreadNode *lchild; // 左孩子/前驱线索
    ThreadNode *rchild; // 右孩子/右驱线索
    int ltag;           // 左标记：0=左孩子，1=前驱线索
    int rtag;           // 右标记：0=右孩子，1=后驱线索
...
class ThreadBinaryTree {
  private:
    ThreadNode *root; // 根节点
    ThreadNode *pre;  // 记录中序遍历的前驱结点（线索化用）

    // 递归构建普通二叉树（测试用）
    ThreadNode *buildTree(int arr[], int idx, int n) {
        if (idx >= n || arr[idx] == -1)
            return nullptr; //-1表示空节点
        ThreadNode *node = new ThreadNode(arr[idx]);
        node->lchild = buildTree(arr, 2 * idx + 1, n); // 左孩子：2i+1
        node->rchild = buildTree(arr, 2 * idx + 2, n); // 右孩子：2i+2
    }

    // 中序线索化核心递归函数
    void inThread(ThreadNode *cur) {
        if (cur == nullptr)
            return;
        ...
        if (cur->lchild == nullptr) {
            cur->ltag = 1;     // 左指针改为前驱线索
            cur->lchild = pre; // 左指针指向前驱结点
        }

        // 3.处理前驱结点的后继线索
        if (pre != nullptr && pre->rchild == nullptr) {
            pre->rtag = 1;     // 前驱的右指针改为后继线索
    ...
    // 线索二叉树中序遍历（非递归，无栈）
    void inOrderTraverse(ThreadNode *p) {
        while (p != nullptr) {
            // 找到中序第一个结点（最左结点，ltag=0时继续左移）
            while (p->ltag == 0) {
                p = p->lchild;
            }
            // 访问当前结点
            cout << p->data << " ";
            // 沿后续线索遍历（rtag=1时直接跳转到后继）
  ...
  public:
    // 构造函数：通过数组构建普通二叉树
    ThreadBinaryTree(int arr[], int n) {
        pre = nullptr;
    ...
    void createInThread() {
        ...
        if (root != nullptr) {
            inThread(root);
            // 处理最后一个结点（无后继，右指针置为线索，指向nullptr）
